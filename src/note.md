...
扫描它们，并构建一个新地图，其中包含专门由源数据制作的元素。它分几个阶段运作
    1 它读取传入的数据。在最初的实现中，这是一个 PNG 文件。在我们的实现中，这是一个 Map 结构，就像我们使用过的其他结构一样；我们还将实现一个 REX Paint 读取器来加载地图。
    2 它将源图像划分为“图块”，并且可以选择通过沿一个或两个轴镜像它读取的图块来制作更多图块。
    3 它加载或构建“约束”图。这是一组规则，指定哪些图块可以彼此相邻。在图像中，这可以从图块邻接导出。在 Roguelike 地图中，出口的连通性是一个很好的指标。对于基于图块的游戏，您可能会仔细构建一个布局，确定什么可以去哪里。
    4 然后它将输出图像划分为平铺大小的块，并将它们全部设置为“空”。放置的第一个图块将是相当随机的，然后它选择区域并检查已知的图块数据 - 放置与已有的图块兼容的图块。最终，它放置了所有的图块 - 你就有了一张地图/图像！
...
在您的 map_builders 目录中创建一个名为 waveform_collapse 的新目录。添加一个文件 mod.rs 到其中。使用类似于前面章节的骨架实现来填充 mod.rs
...
```
修改 rex_assets.rs 嵌入图片作为波函数塌缩 算法的数据源

最后，加载地图本身！在 waveform_collapse 目录中，创建一个新文件： image_loader.rs

现在我们可以修改 build 函数（在 mod.rs 中）来加载地图：

现在可以加载任何 REX Paint 设计的关卡并玩它！如果您现在 cargo run ，您会发现您可以玩新地图：

我们将在后面的章节中将其用于**金库、预制件和预先设计的关卡**- 但现在，我们将仅将其用作稍后波函数塌缩实现的源数据。
