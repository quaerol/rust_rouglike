...
impl BspDungeonBuilder{
    fn new(new_depth)->BspDungeonBuilder{
        BspDungeonBuilder{
    ...
    fn build(&mut self){
        ...
        let stairs = self.rooms[self.rooms.len() - 1];
        let stairs_idx = self.map.xy_idx(stairs);
    ...
    fn add_subrects(&mut self, rects:Rect){
...
impl MapBuilder for BspDungeonBuilder { 
    ...
    fn spawn_entities(&mut self, ecs: &mut World){
        for room in sels.rooms.iter().skip(1) {
